...
ObjectDetector::ObjectDetector(std::string basePath, std::string classesFile, std::string modelConfiguration, std::string modelWeights){
    ...
    vector<int> outLayers = net_.getUnconnectedOutLayers(); // get  indices of  output layers, i.e.  layers with unconnected outputs
    vector<cv::String> layersNames = net_.getLayerNames(); // get  names of all layers in the network
    
    names_.resize(outLayers.size());
    for (size_t i = 0; i < outLayers.size(); ++i) // Get the names of the output layers in names
        names_[i] = layersNames[outLayers[i] - 1];
}

void ObjectDetector::detectObjects(cv::Mat& img, std::vector<BoundingBox>& bBoxes, float confThreshold, float nmsThreshold, bool bVis)
{
    ...
    if(bVis) {
        ...
        for(auto it=bBoxes.begin(); it!=bBoxes.end(); ++it) {
            ...
            label = classes_[((*it).classID)] + ":" + label;
